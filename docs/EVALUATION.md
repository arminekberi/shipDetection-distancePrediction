...
## Reproduce the color comparison

The grayscale experiment above is historical evidence. Active evaluation now
supports only the two color modes. The original three-mode implementation is
preserved in Git commit `23e386c`; the command below runs the color arms only.
...
venv/bin/python evaluate_inputs.py \
  ...
  --out results/full_audit/color_evaluation
```
...
This recording (`20260909-123824`) is in the training split of
`yolo_dataset_v4_color`, so these numbers do not describe checkpoints trained on
that dataset. Labeling noise measured on repeated annotations of identical
frames (median IoU 0.481) also exceeds the F1 differences reported above; see
[DATASET_REPAIR.md](DATASET_REPAIR.md).
