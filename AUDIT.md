...
A small, reviewable patch is provided at `patches/hydrorl-proposer-input.patch`.
...
that repository location; there is no Git checkout in `/root/HydroRL`.
...
4. Train color-input and optional grayscale-augmentation variants with fixed
   split/seed/settings. Compare on the same reviewed test frames. Select by
...
Coasted/tracked boxes do not prove a current YOLO detection. The depth-only,
CSRT-only and older comparison scripts are experimental baselines, not validated
production detectors. Legacy MPEG-4 Part 2 recordings may not play in browsers;
new main/YOLO-only outputs require H.264 support. Existing recorded metadata is
