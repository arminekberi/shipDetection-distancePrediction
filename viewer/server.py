...
Run ``venv/bin/python viewer/server.py`` and open
...
if __package__:
    from .dataset_review import ReviewConflict, ReviewStore
else:
    from dataset_review import ReviewConflict, ReviewStore

PORT = 8001
WIDTH, HEIGHT = 640, 360  # run.py default working size
VIEWER = Path(__file__).resolve().parent
ROOT = VIEWER.parent
ASSETS = {'/control_panel.html', '/panel_data.js', '/dataset_review.html',
          '/dataset_review.js', '/dataset_review.css'}  # served from viewer/
DATA_ROOTS = {'results', 'thumbnails', 'shipcaps_remote', 'shipcaps1_remote'}  # served from ROOT
...
def run_job(video, target):
    ...
    try:
        ...
        cmd += ['--target', target] if target is not None else [
            '--yolo-weights', str(DEFAULT_WEIGHTS), '--tracker', 'bytetrack',
            '--track-occlusion-seconds', '8']
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def translate_path(self, path):
        """Viewer assets live in viewer/, recordings and results under the project root."""
        requested = unquote(urlsplit(path).path)
        return str(VIEWER / requested.lstrip('/')) if requested in ASSETS else super().translate_path(path)
    ...
    def send_head(self):
        ...
        if path not in ASSETS and (not parts or parts[0] not in DATA_ROOTS
    ...
    def do_GET(self):
        if urlsplit(self.path).path.startswith('/api/review/'):
    ...
    def do_POST(self):
        if urlsplit(self.path).path.startswith('/api/review/'):
            return self.review_request(write=True)
    ...
    def review_request(self, write=False):
        host = self.headers.get('Host', '')
        if urlsplit('http://' + host).hostname not in ('127.0.0.1', 'localhost', '::1'):
            return self._send_json({'error': 'Review is available on localhost only'}, 403)
        origin = self.headers.get('Origin')
        if origin and origin != 'http://' + host:
            return self._send_json({'error': 'Cross-origin review requests are not accepted'}, 403)
        route = urlsplit(self.path)
        try:
            store = get_review_store()
            if write:
                if route.path != '/api/review/save':
                    return self._send_json({'error': 'Unknown endpoint'}, 404)
                if self.headers.get_content_type() != 'application/json':
                    return self._send_json({'error': 'Content-Type must be application/json'}, 415)
                ...
                return self._send_json(store.save(json.loads(self.rfile.read(size))))
            if route.path == '/api/review/index':
                return self._send_json(store.index())
            fid = parse_qs(route.query).get('id', [''])[0]
            if route.path == '/api/review/frame':
                return self._send_json(store.detail(fid))
            if route.path == '/api/review/image':
                store.verify_source(fid)
                path = store.image_path(fid)
                body, content_type = path.read_bytes(), self.guess_type(str(path))
            elif route.path == '/api/review/export':
                body, content_type = store.export(), 'application/zip'
            else:
                return self._send_json({'error': 'Unknown endpoint'}, 404)
            ...
            if route.path.endswith('/export'):
                self.send_header('Content-Disposition', 'attachment; filename="annotation-review.zip"')
            self.end_headers()
            self.wfile.write(body)
            ...
            return self._send_json({'error': 'Не удалось прочитать набор или сохранить решение. Проверьте локальные файлы и доступ к диску.'}, 503)
